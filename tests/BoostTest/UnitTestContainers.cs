...
namespace BoostTest
{
    ...
    public class BoostTestContainers
    {
        ...
        static async Task BasicSmokeAsync(UOWTestDb uow, UOWTestDb uow2, UOWTestView uowV)
        {
            ...
            //
            // Test the routine Query
            //
            var vRows = await uowV.GetMyTableRefViewByMyIdAsync(-1);
            Assert.AreEqual(2, vRows.ToList().Count(), "View routine did not return expected number of rows");
            //
            ...
            var rb2 = new DbTest.MyTable { LastChanged = DateTimeOffset.UtcNow, LastChangedBy = "rollback", RowID = sameUniqueGuId };
            try
            {
                await uow.RunInTransactionAsync(async ct =>
                {
        ...
        static Task BasicSmokeSynchronous(UOWTestDb uow, UOWTestDb uow2, UOWTestView uowV)
        {
            ...
            // Test the routine Query
            //
            var vRows = uowV.GetMyTableRefViewByMyIdSynchronized(-1);
            Assert.AreEqual(2, vRows.ToList().Count(), "View routine did not return expected number of rows");
            //
            //Test calling SP/function for retrieving data from a sequence
            //
            ...
            //Bulk- insert & delete tests
            //
            var tt = new DbTest.MyTable { Id = 10, LastChanged = DateTimeOffset.UtcNow, LastChangedBy = "gorm", RowID = Guid.NewGuid() };
            var tt2 = new DbTest.MyTable { Id = 11, LastChanged = DateTimeOffset.UtcNow, LastChangedBy = "gorm2", RowID = Guid.NewGuid() };
