...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = 'BoostScemaX') THEN
        CREATE SCHEMA "BoostScemaX";
    END IF;
END $EF$;
...
COMMENT ON TABLE "BoostScemaX"."IpInfo" IS 'Received IP numbers with reverse lookup data';
...
SELECT setval(
    pg_get_serial_sequence('"BoostScemaX"."IpInfo"', 'Id'),
    GREATEST(
        (SELECT MAX("Id") FROM "BoostScemaX"."IpInfo") + 1,
        nextval(pg_get_serial_sequence('"BoostScemaX"."IpInfo"', 'Id'))),
    false);
